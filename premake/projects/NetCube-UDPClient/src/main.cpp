...
bool initGLFW() {
	...
	window = glfwCreateWindow(800, 800, "Client", nullptr, nullptr);
...
float tx = 0.0f;
float ty = 0.0f;
float updateInterval = 0.100f; // Make the update interval a variable
...
void keyboard() {
	...
	if (glfwGetKey(window, GLFW_KEY_A) == GLFW_PRESS) {
		updateInterval += 0.001; // Add to update interval
	}
	if (glfwGetKey(window, GLFW_KEY_Z) == GLFW_PRESS) {
		updateInterval -= 0.001; // Remove from update interval
	}
...
#define PORT "5000"
#define BUFLEN 512
#define UPDATE_INTERVAL updateInterval //seconds
...
int main() {
	...
	while (!glfwWindowShouldClose(window)) {
		...
		//time -= delta;
		if (updateGame == true) // Only send an update when the game should be updated (when the box is being moved)
		{
			...
			if (sendto(client_socket, message, BUFLEN, 0, ptr->ai_addr, ptr->ai_addrlen) == SOCKET_ERROR)
				std::cout << "Sendt() failed..." << std::endl;
